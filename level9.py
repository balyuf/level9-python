...
def _printMessage(data, msgAddress, multi = True):
    ...
    for character in message:
        ...
        elif(messageVersion >= 3 and character == ' ' and charsWrittenToLine==0):
            print()
        elif(character != nl):
            print(character, end='')
            charsWrittenToLine += 1
...
def _printAllExits(data,exitsAddr):
    ...
    match game:
        case ("secret" | "secretp2" | "secreptp3" | "secretp4" |
                            ...
                            if 'code' in lastMessage:
                                ...
                                if inputCycles < 1024:
                                    ...
                                    while len(c) > 1 and c[0] == '#':
                                        _process_hash_commands(data, pc, c)
                                        c = _scriptInput()
                                    if not c:
                                        c = 0
                                    else:
                    ...
                    case 11: #load_next_part
                        ...
                        if (oldGame in v1Configuration and 'parts' in v1Configuration[oldGame] and
                           v1Configuration[oldGame]["filename"] == filename):
                            if not part:
                                part = v1Configuration[oldGame]['parts'].index(game) + 2
                            game     = v1Configuration[oldGame]['parts'][part - 1]
