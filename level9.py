...
def _printMessage(data, msgAddress, multi = True):
    ...
    for character in message:
        ...
        elif(character == ' ' and charsWrittenToLine==0):
            print()
        elif(character != nl):
            print(character, end='')
            charsWrittenToLine += 1
...
def _printAllExits(data,exitsAddr):
    ...
    match game:
        ...
        case "eden" | "edenv3":
            ...
            print("Bit 2 - if there is a door in this direction")
            print("\n")
        case "snowball" | "snowballv2" | "snowballv3":
        ...
        case "lords" | "lordsv2" | "lordsv4":
            ...
            print("Bit 2 - if there is a door in this direction")
            print("\n")
        case "adventure" | "adventurev2" | "adventurev3":
            ...
            print("\n")
        case "dungeon" | "dungeonv2" | "dungeonv3":
            ...
            print("Bit 2 - not used, always set to 0x00 but printed here as 'No'")
            print("\n")
        case "colossal" | "colossalv2" | "colossalv3":
...
def _build_message(message, word, multi = False):
    ...
    if prevMessage and word:
        ...
        if word[0] not in nextChars and prevMessage[-1] not in prevChars:
            word = ' ' + word
        if re.search("[.?!][ (>'\"\n]*$", prevMessage): # or (multi and not charsWrittenToLine):  XXX too many!
...
def vm_fn_function(data,opCode,pc):
    ...
    global list2Offset, list3Offset, list9Offset
    pc=pc+1
    ...
    match requiredFunction:
            ...
            if version < 3:
                ...
                match vm_listarea[list9Offset]:
                    ...
                    case 1: #checksum
                        pass
                    case 2: #print_char
                        print(chr(vm_listarea[list9Offset + 1]))
                    case 3: #input_char
                        if scriptFile:
                            if 'code' in lastMessage:
                                vm_listarea[list9Offset + 1] = ord(lenslok[lens])
                                lens = (lens + 1) % 2
                            elif 'SPACE' in lastMessage or 'space' in lastMessage:
                    ...
                    case 11: #load_next_part
                        ...
                        global vm_dictionary, vm_breakpoints, vm_return_breakpoints, charsWrittenToLine
                        part = vm_listarea[list9Offset + 1]
                        if game in v1Configuration and 'parts' in v1Configuration[game]:
                            if not part:
                                part = v1Configuration[game]['parts'].index(game) + 2
                            game     = v1Configuration[game]['parts'][part - 1]
                        ...
                        with open(filename, 'rb') as dataFile:
                            ...
                            globals()["data"] = data
                            dataFile.close()
                        aCodeStartAddr, foundGame, _ = _autodetect_game(data, version)
                        if game == 'unknown':
                            game = foundGame
                        if args.game and game != 'unknown':
                            args.game = game
                        ...
                        if game == 'unknown':
                            game = _identify_game(data, version)
        ...
        case 250:
            if version < 3:
                print(f'Error: Function - Print String ({requiredFunction:02x}) not supported in version '+version)
                sys.exit()
            pc=pc+1
            operand=''
    ...
    if version <= 2:
        ...
        if not inputLine and not inputString:
            ...
            if len(inputLine) > 0 and inputLine[0] == '#':
                _process_hash_commands(data, pc, inputLine)
                inputLine = ''
                ...
                if term.lower().startswith(inputString):
                    ...
                    if msgCodes:
                        if not abrevDict:
                            ...
                            #print(f'found ambiguous abrevCodes: {list(map(lambda t: tuple(map(hex, t)), msgCodes))}')
                            msgCodes = []
                            found = False
                        else:
                            #print(f'found first abrevCodes: {list(map(lambda t: tuple(map(hex, t)), msgCodes))}')
                            abrevCodes = msgCodes
                            abrevMsg = True
                            found = True
            if found and abrevMsg:
                msgCodes = abrevCodes
                #print(f'found final abrevCodes: {list(map(lambda t: tuple(map(hex, t)), msgCodes))}')
        else:
...
#   game indicator
###############################################################################
def _identify_game(data, version):
    game = "unknown"
    msgs = [0x01, 0xa0, 0xe6, 0xff]
    if version >= 3:
        msgs += [0x44c, 0x4f6, 0x834]
...
if(args.script):
    ...
    if game in v1Configuration:
        v1Configuration[game]['script'] = args.script
